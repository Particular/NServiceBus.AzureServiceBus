namespace NServiceBus.AzureServiceBus.Tests
{
    ...
    public class When_retrying_on_throttle
    {
        ...
        public void Non_server_busy_exceptions_will_bubble_up_without_retry()
        {
            ...
            //validate
            Assert.Throws<NotSupportedException>(async () => await sender.RetryOnThrottle(s => s.SendAsync(new BrokeredMessage()), TimeSpan.FromSeconds(10), 5));
            Assert.IsTrue(sender.IsInvoked);
            Assert.AreEqual(1, sender.InvocationCount);
        }
        ...
        public void Server_busy_exceptions_will_bubble_up_with_retry()
        {
            ...
            Assert.Throws<ServerBusyException>(async () => await sender.RetryOnThrottle(s => s.SendAsync(new BrokeredMessage()), TimeSpan.FromSeconds(1), 5));
        ...
        public async Task Server_busy_exceptions_will_not_bubble_up_after_successfull_retry()
        {
            ...
            await sender.RetryOnThrottle(s => s.SendAsync(new BrokeredMessage()), TimeSpan.FromSeconds(1), 5);
        ...
        public void Server_busy_exceptions_will_delay_retry()
        {
            ...
            Assert.Throws<ServerBusyException>(async () => await sender.RetryOnThrottle(s => s.SendAsync(new BrokeredMessage()), TimeSpan.FromSeconds(5), 1));
