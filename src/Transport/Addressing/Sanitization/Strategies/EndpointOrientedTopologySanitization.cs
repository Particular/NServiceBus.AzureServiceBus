        ...
        string ValidateAndSanitizeLengthOfEntityPathOrName(string entityPath, EntityType entityType)
        {
            ...
            switch (entityType)
            {
                case EntityType.Queue:
                    shouldHash = entityPath.Length > settings.GetOrDefault<int>(WellKnownConfigurationKeys.Topology.Addressing.Sanitization.QueuePathMaximumLength);
                    break;

                case EntityType.Topic:
                    shouldHash = entityPath.Length > settings.GetOrDefault<int>(WellKnownConfigurationKeys.Topology.Addressing.Sanitization.TopicPathMaximumLength);
                    break;

                case EntityType.Subscription:
                    shouldHash = entityPath.Length > settings.GetOrDefault<int>(WellKnownConfigurationKeys.Topology.Addressing.Sanitization.SubscriptionPathMaximumLength);
                    break;

                case EntityType.Rule:
                    shouldHash = entityPath.Length > settings.GetOrDefault<int>(WellKnownConfigurationKeys.Topology.Addressing.Sanitization.RuleNameMaximumLength);
                    break;
